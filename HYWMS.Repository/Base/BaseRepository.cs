using HYWMS.Repository.UnitOfWork;
using SqlSugar;
using System;
...
namespace HYWMS.Repository.Base
{
    public class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        /// <returns></returns>
        public async Task<int> AddList_Bulk(List<T> models)
        {
            return await _db.Fastest<T>().BulkCopyAsync(models);
        }
        ...
        public async Task<List<T>> ExcuteQuerySql(string sql)
        {
            return await _db.Ado.SqlQueryAsync<T>(sql);
        }


        public async Task<List<T2>> SqlQueryToDiyModAsync<T2>(string strSql, object paras = null)
        {
            return await _db.Ado.SqlQueryAsync<T2>(strSql);
        }
